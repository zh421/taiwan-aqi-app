...
def getlogin(username, password):
    ...
    if validation:
        ...
        # 顯示隱藏的root window
        root.deiconify()
        # login window不會再用，銷毀 -> 不能用destroy()，因為new window 使用Toplevel，保證widgets可行，使用destroy 也會close child window
        login.withdraw()
    else:
        ...
        with open("info/credentials.txt", "r") as credentials:
            for line in credentials:
                ...
                try:
                    ...
                    print("You need to Register first or check file info/credentials.txt")
                    return False
            return False
...
class LoginPage(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        label_title = tk.Label(frame_login, st.title_styles, text="Login Page")
        label_title.grid(row=0, column=1, columnspan=1)

        label_user = tk.Label(frame_login, st.text_styles, text="Username:")
        label_user.grid(row=1, column=0)

        label_pw = tk.Label(frame_login, st.text_styles, text="Password:")
...
root = TkinterTemplate.MyApp(login) # 為了確實的關閉app，要把啟動的畫面也傳進去
